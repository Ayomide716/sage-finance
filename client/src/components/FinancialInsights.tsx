    ...
    const generateInsights = async () => {
      ...
      if (overBudgetItems.length > 0) {
        const worstOffender = overBudgetItems.reduce((prev, current) => 
          (current.spent - current.amount) > (prev.spent - prev.amount) ? current : prev
        );
        
        generatedInsights.push({
          type: 'warning',
          title: `${worstOffender.category} Budget Alert`,
          message: `You've exceeded your ${worstOffender.category.toLowerCase()} budget by $${(worstOffender.spent - worstOffender.amount).toFixed(2)} this month.`,
          icon: 'warning'
        });
      }
